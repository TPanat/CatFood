 ...
 const toggleComparison = (id) => {
  setComparisonList(prevList => {
   if (prevList.includes(id)) {
    ...
    showToast('สามารถเลือกเปรียบเทียบได้สูงสุด 4 รายการเท่านั้น');
    return prevList;
   }
  });
 };
 ...
 const isComparing = (id) => comparisonList.includes(id);
  ...
  const sortedAndFilteredFood = useMemo(() => {
    ...
    let currentData = catFoodData.filter(food => {
      const typeMatch = filterType.length === 0 || filterType.some(ft => {
        if (ft === 'Prescription') return food.type.toLowerCase().includes('prescription');
        // Match base type: 'Wet' matches 'Wet', 'Wet (Pouch)', 'Wet (Can)', etc.
        // But avoid 'Dry' matching 'Dry (Prescription)' when Prescription is a separate filter
        return food.type.startsWith(ft) && !food.type.toLowerCase().includes('prescription');
      });
   ...
   <h1 className={styles.pageTitle}>
    ...
    <div className={styles.infoSection}>
     ...
     <div className={styles.infoContent}>
      <p>
        เพื่อให้การเปรียบเทียบระหว่างอาหารเม็ด (Dry) และอาหารเปียก (Wet) เป็นไปอย่างเที่ยงตรง ตารางเปรียบเทียบจะใช้ค่า <strong>Dry Matter Basis (DMB)</strong> สำหรับสารอาหารหลัก (โปรตีน ไขมัน และใยอาหาร)
      </p>
      <h3>🔬 Dry Matter Basis (DMB) คืออะไร?</h3>
      <ul>
        <li>ค่า <strong>As Fed</strong> (ค่าที่ระบุบนฉลาก) คือสัดส่วนสารอาหารในอาหาร &ldquo;ตามที่เห็น&rdquo; ซึ่งรวมน้ำเข้าไปด้วย</li>
        <li>ค่า <strong>DMB</strong> คือสัดส่วนสารอาหาร <strong>หลังจากที่เอาน้ำออกไปหมดแล้ว</strong> ซึ่งทำให้เราสามารถเปรียบเทียบโภชนาการที่แมวได้รับจริงจากอาหารต่างประเภทกันได้อย่างยุติธรรม</li>
      </ul>
      ...
      <p style={{ marginTop: '15px', fontSize: '0.9em', color: '#888' }}>
        <strong>หมายเหตุ:</strong> ในตารางเปรียบเทียบ จะมีการไฮไลต์ค่าที่ดีที่สุด (โปรตีน/ไขมัน/ทอรีนสูงสุด และความชื้นต่ำสุด) เพื่อช่วยในการตัดสินใจ
      </p>
     </div>
    </div>
